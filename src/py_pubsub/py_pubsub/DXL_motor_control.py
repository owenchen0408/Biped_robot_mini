...
class DXL_Conmunication(object):
    def __init__(self, device_name = '/dev/ttyUSB0', b_rate=3000000):
        BAUDRATE                    = b_rate             # Dynamixel default baudrate : 57600
...
class DXL_motor(object):
        ...
        if not self.torqueEnabled():
            ...
            if switch_success:
                ...
                if self.OPERATING_MODE == mode:
                    print("Motor {0} OP Mode chenge to {1} successfully".format(
                        self.DXL_ID, mode
                    ))
                    return True
                else:
                    print("Motor {0} OP Mode chenge unsuccessfully".format(
                        self.DXL_ID
                    ))
                    return False
        else:
    ...
    def readHardwareError(self):
        ...
        print("Motor {0} with hardware error code: {1}".format(self.DXL_ID, value))
    ...
    def directReadData(self, add, len, print_msg=True)->(int,bool):
