...
class RosTopicSubscriber(Node):
    ...
    def twist_callback(self, msg):
        if HeightCount<=5:
            self.linear_vel = msg.linear.x * 10
        else:
            self.linear_vel = msg.linear.x * 4
...
class robotcontrol:
    ...
    def frontStandUp(self):
        ...
        #self.enableAllMotor()
        self.dxl.updateMotorData()
        self.motor02.writePosition(3000)  # 2760-1795
        self.dxl.sentAllCmd()
        _ = self.mc.torquecontrol(0x01, 300)
        # _ = self.mc.torquecontrol(0x02, -300)
        time.sleep(1)
        self.motor12.writePosition(3000)  # 2760-1795
        self.dxl.sentAllCmd()
        time.sleep(1)
...
#X mode
            ...
            if HeightCount<=15:
                self.setAngularPI(65,0)
            else:
                self.setAngularPI(75, 5)
    ...
    while True:
        try:
            ...
            if cmd in command_dict:
                ...
                robot_motor.changeHeight(dy=0.01)
            elif cmd == "k":
                robot_motor.changeHeight(dy=-0.01)
            elif cmd == "j":
                robot_motor.changeHeight(dx=0.01)
            elif cmd == "l":
                robot_motor.changeHeight(dx=-0.01)
            elif cmd == "exit":
